...
public class Weather_Controller : Singleton<Weather_Controller>
{
    /********** ----- VARIABLES ----- **********/
    ...
    public void ExitCurrentWeather(int NewWeatherType)
    {
        ...
        else if (en_CurrWeather == WeatherType.THUNDERSTORM)
        {
            ...
            Debug.Log(_fTimeChangeWeatherStart + " SHOULD BE GREATER THAN " + _fTimeChangeWeatherEnd);
            // After we reached or set time, we start to change into the new weather type
            if (_fTimeChangeWeatherStart >= _fTimeChangeWeatherEnd)
            {
