...
public class ToD_Base : MonoBehaviour 
{
    ...
    void Start()
    {
        _fStartingHour = ONEHOURLENGTH * DateTime.Now.Hour;
        _fCurrentTimeOfDay = _fStartingHour;

        Debug.Log("CURRENT TIME IS : " + _fCurrentTimeOfDay);

        _fStartingSunrise = ONEHOURLENGTH * (float)CovertCurrentWeatherToWeatherSystem.Instance.sunriseTime.Hour + (CovertCurrentWeatherToWeatherSystem.Instance.sunriseTime.Minute / 60f);
        _fStartingDay = ONEHOURLENGTH * (float)_iDayStart;
        _fStartingSunset = ONEHOURLENGTH * (float)CovertCurrentWeatherToWeatherSystem.Instance.sunsetTime.Hour + (CovertCurrentWeatherToWeatherSystem.Instance.sunsetTime.Minute / 60f);
